// ! INDICATES THE CURRENT TARGET
export const TARGET = "joesguns";

export const MAX_PORTS_REQUIRED = 2;

// Host Names
export const HOME_HOST = "home";
export const LEVEL_0_HOSTS = [
  "n00dles",
  "foodnstuff",
  "sigma-cosmetics",
  "joesguns",
  "hong-fang-tea",
  "harakiri-sushi",
];
export const LEVEL_1_HOSTS = ["iron-gym"];

// Script Paths
export const C_HACK_SCRIPT_PATH = "/scripts/c-hack.js";
export const C_HACK_ALL_PATH = "/scripts/c-hack-all.js";
export const COPY_SCRIPTS_PATH = "/scripts/copy-scripts.js";
export const OMNI_HACK_PATH = "/scripts/omni-hack.js";

// Program Paths
export const BRUTESSH_PATH = "BruteSSH.exe";
export const FTPCRACK_PATH = "FTPCrack.exe";
